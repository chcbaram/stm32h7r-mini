...
static const button_hw_t button_hw[BUTTON_MAX_CH] =
    {
      {GPIOO, GPIO_PIN_2, GPIO_PIN_RESET, NAME_DEF(BTN_CH1)},      
    };
...
bool buttonGetPin(uint8_t ch)
{
  ...
  if (HAL_GPIO_ReadPin(button_hw[ch].port, button_hw[ch].pin) == button_hw[ch].on_state)
  {
    ret = true;
  }
...
void cliButton(cli_args_t *args)
{
  ...
  if (args->argc == 1 && args->isStr(0, "info"))
  {
    for (int i=0; i<BUTTON_MAX_CH; i++)
    {
      cliPrintf("%-12s pin %d : %d\n", buttonGetName(i), button_hw[i].pin, buttonGetPressed(i));
    }
    ret = true;
  }
