...
void cliFlash(cli_args_t *args)
{
  ...
  if (args->argc == 3 && args->isStr(0, "xip-read") == true)
  {
    uint32_t *p_data;
    float     length_mb;
    uint32_t i;
    ...
    length_mb = args->getFloat(2);
    test_length = (uint32_t)(length_mb * 1024 * 1024);

    cliPrintf("Addr..   0x%X\n", args->getData(1));
    cliPrintf("Length.. %3.2f MB\n", length_mb);
  ...
  if (ret == false)
  {
    ...
    cliPrintf( "flash xip-read [addr] [size MB]\n");
  }
}
#endif
