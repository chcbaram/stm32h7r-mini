...
uint8_t qspiHwErase_Block(uint32_t block_addr);
uint8_t qspiHwErase_Sector(uint32_t SectorAddress);
uint8_t qspiHwErase_Chip (void);
uint8_t qspiHwGetStatus(void);
uint8_t qspiHwGetInfo(QSPI_Info* p_info);
uint8_t qspiHwEnableMemoryMappedMode(void);
uint8_t qspiHwGetID(QSPI_Info* p_info);
uint8_t qspiHwConfig(void);
uint8_t qspiHwReset(void);
uint8_t qspiHwAbort(void);
...
bool qspiRead(uint32_t addr, uint8_t *p_data, uint32_t length)
{
  ...
  ret = qspiHwRead(p_data, addr, length);
...
bool qspiWrite(uint32_t addr, uint8_t *p_data, uint32_t length)
{
  ...
  ret = qspiHwWrite(p_data, addr, length);
...
bool qspiEraseBlock(uint32_t block_addr)
{
  ...
  ret = qspiHwErase_Block(block_addr);
...
bool qspiEraseSector(uint32_t sector_addr)
{
  ...
  ret = qspiHwErase_Sector(sector_addr);
...
bool qspiErase(uint32_t addr, uint32_t length)
{
  ...
  flash_length = W25Q128JV_FLASH_SIZE;
  block_size   = W25Q128JV_SECTOR_SIZE;
...
bool qspiGetInfo(qspi_info_t* p_info)
{
  ...
  ret = qspiHwGetInfo((QSPI_Info *)p_info);
...
bool qspiEnableMemoryMappedMode(void)
{
  ...
  ret = qspiHwEnableMemoryMappedMode();
...
uint8_t qspiHwInit(void)
{
  ...
  hqspi.Init.MemorySize              = HAL_XSPI_SIZE_128MB;
...
uint8_t qspiHwWrite(uint8_t* p_data, uint32_t addr, uint32_t length)
{
  ...
  current_size = W25Q128JV_PAGE_SIZE - (addr % W25Q128JV_PAGE_SIZE);
...
uint8_t qspiHwErase_Block(uint32_t BlockAddress)
{
  XSPI_RegularCmdTypeDef s_command = {0};
  ...
  if (QSPI_AutoPollingMemReady(&hqspi, W25Q128JV_SUBSECTOR_ERASE_MAX_TIME) != QSPI_OK)
  {
    return QSPI_ERROR;
  }
...
uint8_t qspiHwErase_Chip(void)
{
  ...
  /* Configure automatic polling mode to wait for end of erase */
  if (QSPI_AutoPollingMemReady(&hqspi, W25Q128JV_BULK_ERASE_MAX_TIME) != QSPI_OK)
  {
    return QSPI_ERROR;
  }
...
uint8_t qspiHwGetInfo(QSPI_Info* p_info)
{
  ...
  p_info->FlashSize          = W25Q128JV_FLASH_SIZE;
  p_info->EraseSectorSize    = W25Q128JV_SUBSECTOR_SIZE;
  p_info->EraseSectorsNumber = (W25Q128JV_FLASH_SIZE/W25Q128JV_SUBSECTOR_SIZE);
  p_info->ProgPageSize       = W25Q128JV_PAGE_SIZE;
  p_info->ProgPagesNumber    = (W25Q128JV_FLASH_SIZE/W25Q128JV_PAGE_SIZE);
...
static uint8_t QSPI_WriteEnable(XSPI_HandleTypeDef *p_hqspi)
{
  ...
  s_config.MatchValue      = W25Q128JV_SR_WREN;
  s_config.MatchMask       = W25Q128JV_SR_WREN;
...
static uint8_t QSPI_AutoPollingMemReady(XSPI_HandleTypeDef *p_hqspi, uint32_t Timeout)
{
  ...
  s_config.MatchMask       = W25Q128JV_SR_WIP;
  s_config.MatchMode       = HAL_XSPI_MATCH_MODE_AND;
  s_config.IntervalTime    = 0x10;
  s_config.AutomaticStop   = HAL_XSPI_AUTOMATIC_STOP_ENABLE;
