...
bool spiBegin(uint8_t ch)
{
  ...
  switch(ch)
  {
    case _DEF_SPI1:
      p_spi->h_spi = &hspi1;      
      p_spi->h_dma_rx = &handle_GPDMA1_Channel1;
      ...
      p_spi->h_spi->Init.BaudRatePrescaler= SPI_BAUDRATEPRESCALER_4; // 4=75Mhz
      ...
      HAL_SPI_RegisterCallback(p_spi->h_spi, HAL_SPI_TX_COMPLETE_CB_ID, spiTxCpltCallback);
      HAL_SPI_RegisterCallback(p_spi->h_spi, HAL_SPI_RX_COMPLETE_CB_ID, spiRxCpltCallback);
...
void spiTxCpltCallback(SPI_HandleTypeDef *hspi)
{
  for (int i=0; i<SPI_MAX_CH; i++)
  {
